...
1. **Presentation (`src/presentation/`):** Handles HTTP requests/responses, routes, and controllers. No business logic here. *Rule:* Delegate all error-to-status mapping to the shared HTTP response helper. All controllers MUST use `next(error)` for error handling—never inline error responses.
2. **Business Logic (`src/business/`):** Pure functional services, pathfinding algorithms, domain rules. *Rule:* Contains mapping functions (`toApiShape`/`toDbShape`). *Rule:* Cross-entity validations (e.g. checking FK existence) and uniqueness validations (e.g. checking if an email already exists to return a 409 Conflict) MUST occur here by explicitly invoking the target entity's repository before taking action. Do not rely on passively catching DB constraint errors. Error types MUST be created via the existing error factory (`createAppError`)—never throw raw `new Error()` from a service. *Note:* `src/business/pathfinder.js` is intentionally a placeholder; Phase 5B will implement the real algorithm inside `calculatePath` without changing its signature or any other file.
...
4. **Utilities (`src/utils/`):** Shared pure functions (like the Error Factory and HTTP Response Helper) to eliminate duplication. *Rule:* All logging MUST go through `src/utils/logger.js`—never use `console.log`.
